...
;(check-equal? (substitute "x" 5 (string->expr (append "(" (append (expr->string (substitute "y" 3 (substitute "x" "y" (string->expr "(x add y)")))) (append " add x)" '())))))  (string->expr (expr->string (make-bin-expr (make-bin-expr 3 "add" 3) "add" 5))) )
